...
const EmployeeToDeleteList = props => {
	...
	const EmployeeItem = props => {
		...
		return (
			<ItemLi>
				{' '}
				{props.name} <ItemButton onClick={() => deleteEmployee(props.name)}>DELETE</ItemButton>
			</ItemLi>
		);
	};
	...
	return (
		<section>
			<H3WithReload>
				Delete an employee{' '}
				<ReloadImgTag onClick={() => setIsEmployeeListLoaded(false)} src={realodImg} alt='reload' />{' '}
			</H3WithReload>
			<ul>{nameList ? <EmployeeListItem list={nameList} /> : null}</ul>
		</section>
	);
};
